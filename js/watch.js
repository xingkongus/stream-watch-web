var HOST = "xingkongus.gqt.gcu.edu.cn";
//获取URL GET参数
var GET = getGET();
function getGET() {
...
var ajaxObj;
if(app != null){
	ajaxObj = $.ajax({url:"http://" + HOST + "/?s=/index/user/app&app=" + app,async:false,dataType: 'json'});//同步加载app
...
if(group != null){
	ajaxObj = $.ajax({url:"http://" + HOST + "/?s=/index/user/group&group=" + group,async:false,dataType: 'json'});//同步加载app
...
//初始化弹幕客户端
client = new BucketClient(HOST,80,"websocket/" + (group || app));
